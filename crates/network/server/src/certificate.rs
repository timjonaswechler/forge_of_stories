...
use network_shared::certificate::CertificateFingerprint;
...
fn generate_self_signed_certificate(
    ...
    let generated = rcgen::generate_simple_self_signed(vec![server_host.clone()])?;
    ...
    match cert_mode {
        ...
        CertificateRetrievalMode::LoadFromFileOrGenerateSelfSigned {
            ...
            if Path::new(&cert_file).exists() && Path::new(&key_file).exists() {
                ...
                warn!(
                    "{} and/or {} do not exist, could not load existing certificate. Generating a new self-signed certificate.",
                    cert_file, key_file
                );
