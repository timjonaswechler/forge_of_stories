...
pub const CHANNEL_ID_LEN: usize = 1;
pub const PROTOCOL_HEADER_LEN: usize = CHANNEL_ID_LEN;
pub type CloseSend = broadcast::Sender<CloseReason>;
pub type CloseRecv = broadcast::Receiver<CloseReason>;
...
#[derive(Debug)]
pub enum ChannelAsyncMessage {
    LostConnection,
}

#[derive(Debug)]
pub enum ChannelSyncMessage {
    CreateChannel {
        id: ChannelId,
        kind: ChannelKind,
...
impl Channel {
    pub fn new(id: ChannelId, sender: mpsc::Sender<Bytes>, close_sender: mpsc::Sender<()>) -> Self {
        Self {
            id,
            sender,
            close_sender,
    ...
    pub fn send_payload(&self, payload: Bytes) -> Result<(), AsyncChannelError> {
...
impl ChannelsConfiguration {
    ...
    pub fn configs(&self) -> &Vec<ChannelKind> {
        &self.channels
    }
}

/// Spawn a task to handle send channels creation for this connection
pub fn spawn_send_channels_tasks_spawner(
...
pub async fn send_channels_tasks_spawner(
    connection: quinn::Connection,
