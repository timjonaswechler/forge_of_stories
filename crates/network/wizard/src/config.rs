...
impl Config {
    pub fn new() -> Result<Self, config::ConfigError> {
        ...
        // Merge default styles (string keyed scopes)
        for (scope, default_styles) in default_config.styles.0.iter() {
            let user_styles = cfg.styles.0.entry(scope.clone()).or_default();
...
impl<'de> Deserialize<'de> for Styles {
    ...
    where
        D: Deserializer<'de>,
    {
        // Deserialize into a string-keyed map first (scope -> map of style name -> style string)
        ...
        let styles = parsed_map
            ...
            .map(|(scope, inner_map)| {
                let converted_inner_map = inner_map
                    .into_iter()
                    .map(|(k, v)| (k, parse_style(&v)))
                    .collect();
                (scope, converted_inner_map)
            })
            .collect();
