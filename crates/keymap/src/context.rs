//! Key context system for hierarchical action dispatch.
//!
//! Contexts represent the current state of the UI and are used to determine
//! which key bindings should be active. They are arranged in a stack from
//! lowest (root) to highest (focused element).

use anyhow::{Context as _, Result};
use std::fmt;

/// A context entry with an optional value.
...
/// and/or key value pairs representing the current context for the keymap.
...
impl KeyContext {
    ...
    pub fn add<I: Into<String>>(&mut self, identifier: I) {
    ...
    pub fn set<S1: Into<String>, S2: Into<String>>(&mut self, key: S1, value: S2) {
        let key = key.into();
        if !self.contains(&key) {
    ...
    pub fn get(&self, key: &str) -> Option<&String> {
        self.0.iter().find(|entry| entry.key == key)?.value.as_ref()
    }
}
    ...
    fn parse_primary(mut source: &str) -> Result<(Self, &str)> {
        ...
        match next {
            ...
            _ if is_identifier_char(next) => {
                ...
                Ok((
                    KeyBindingContextPredicate::Identifier(identifier.to_string()),
                    source,
                ))
            }
...
mod tests {
    ...
    fn test_parse_identifiers() {
        assert_eq!(
            ...
            Identifier("abc12".to_string())
        );
        assert_eq!(
            KeyBindingContextPredicate::parse("_1a").unwrap(),
            Identifier("_1a".to_string())
        );
    }
    ...
    fn test_parse_negations() {
        assert_eq!(
            ...
            Not(Box::new(Identifier("abc".to_string())))
        );
        assert_eq!(
            KeyBindingContextPredicate::parse(" ! ! abc").unwrap(),
            Not(Box::new(Not(Box::new(Identifier("abc".to_string())))))
        );
    }
    ...
    fn test_parse_equality_operators() {
        ...
        assert_eq!(
            ...
            NotEqual("c".to_string(), "d".to_string())
        );
    }
