...
impl Endpoint {
    ...
    pub(crate) fn try_recv_async_message(&mut self) -> Option<ServerAsyncMessage> {
        match self.from_async_endpoint_recv.try_recv() {
            Ok(message) => Some(message),
            Err(TryRecvError::Empty) | Err(TryRecvError::Disconnected) => None,
        }
    }

    pub(crate) fn poll_channel_messages<F>(&mut self, mut f: F)
    where
        F: FnMut(ClientId, ChannelAsyncMessage),
    {
    ...
    pub(crate) fn channel_kind(&self, channel_id: ChannelId) -> Option<ChannelKind> {
        self.opened_channels.get(&channel_id).copied()
    }
        ...
        match self.receive_payload_from(client_id)? {
            Some((channel_id, payload)) => {
                match bincode::serde::decode_from_slice::<T, _>(
                    ...
                    Ok((msg, _)) => Ok(Some((channel_id, msg))),
    ...
    >(
        ...
        let payload = match bincode::serde::encode_to_vec(message, bincode::config::standard()) {
            Ok(p) => p,
            Err(_) => return Err(ServerGroupMessageSendError::Serialization),
        };
        ...
        if errs.is_empty() {
            ...
            Err(ServerGroupSendError(errs).into())
        }
    }
