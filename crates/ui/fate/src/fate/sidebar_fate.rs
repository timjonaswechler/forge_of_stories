...
use gpui::{
    Action, App, AppContext, ClickEvent, Context, Entity, Focusable, Hsla, IntoElement,
    ParentElement, Render, SharedString, Styled, Window, div, prelude::FluentBuilder, relative,
};
...
impl Item {
    ...
    pub fn handler(
        &self,
    ) -> impl Fn(&mut FateSidebar, &ClickEvent, &mut Window, &mut Context<FateSidebar>) + 'static
    {
        let item = *self;
        move |this, _, _, cx| {
...
impl Focusable for FateSidebar {
    fn focus_handle(&self, _: &gpui::App) -> gpui::FocusHandle {
        self.focus_handle.clone()
    }
}

impl Render for FateSidebar {
    fn render(
        &mut self,
