...
use crate::{
    action::Action,
    components::{popup::PopupComponent, Component},
    tui::Frame,
    theme::{Theme, UiGroup},
};
...
impl Component for KeymapOverlay {
    fn name(&self) -> &'static str { "keymap_overlay" }
    ...
    fn draw(&mut self, f: &mut Frame<'_>, area: Rect) -> Result<()> {
        ...
        let overlay = Rect { x: area.x, y: area.y + area.height - overlay_h, width: area.width, height: overlay_h };
        ...
        let block = Block::bordered()
            ...
            .title(Span::styled(format!(" {} ", self.title), theme.style(UiGroup::Title)));
        ...
        for (i, (label, chords)) in self.entries.iter().enumerate() {
            ...
            let lstyle = if is_sel { theme.style(UiGroup::ModeNormal) } else { theme.style(UiGroup::Statusline) };
            left.push(Line::from(Span::styled(label.clone(), lstyle)));
            let chord_line = chords.join(" / ");
            right.push(Line::from(Span::styled(chord_line, theme.style(UiGroup::Dimmed))));
        }

        f.render_widget(Paragraph::new(Text::from(left)).wrap(ratatui::widgets::Wrap { trim: true }), cols[0]);
        f.render_widget(Paragraph::new(Text::from(right)).wrap(ratatui::widgets::Wrap { trim: true }), cols[1]);
        Ok(())
    }

    fn handle_key_events(&mut self, key: KeyEvent) -> Result<Option<crate::tui::EventResponse<Action>>> {
        match key.code {
            KeyCode::Up => {
                if self.selected == 0 { self.selected = self.entries.len().saturating_sub(1); }
