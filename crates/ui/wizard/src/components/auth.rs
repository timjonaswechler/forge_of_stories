use crate::{action::Action, components::Component, services::auth, state::State};
...
impl AuthComponent {
    ...
    fn submit(&mut self) -> Result<Option<Action>> {
        ...
        match self.mode {
            ...
            Mode::Login => {
                ...
                self.clear_inputs();
                self.info = None;
                // TODO: Navigation, sobald Setup-Page existiert:
                // return Ok(Some(Action::Navigate(…)));
                return Ok(None);
            }
        }
    }
    ...
    fn render_info(&self, frame: &mut Frame, area: Rect) {
        let (msg, style) = if let Some(err) = &self.error {
            ...
            (info.as_str(), Style::default().fg(Color::Green))
        } else {
...
impl Component for AuthComponent {
    ...
    fn handle_key_events(
        ...
        key: KeyEvent,
        _state: &mut State,
    ) -> color_eyre::Result<Option<crate::tui::EventResponse<Action>>> {
        match key.code {
            KeyCode::Tab => {
                self.focus = match (self.mode, self.focus) {
                    (Mode::CreateAdmin, Focus::Username) => Focus::Password,
                    (Mode::CreateAdmin, Focus::Password) => Focus::Confirm,
                    (Mode::CreateAdmin, Focus::Confirm) => Focus::Username,
                    (Mode::Login, Focus::Username) => Focus::Password,
                    (Mode::Login, Focus::Password) => Focus::Username,
                    _ => Focus::Username,
                };
                self.error = None;
                Ok(None)
            ...
            KeyCode::BackTab => {
                self.focus = match (self.mode, self.focus) {
                    (Mode::CreateAdmin, Focus::Username) => Focus::Confirm,
                    (Mode::CreateAdmin, Focus::Confirm) => Focus::Password,
                    (Mode::CreateAdmin, Focus::Password) => Focus::Username,
                    (Mode::Login, Focus::Username) => Focus::Password,
                    (Mode::Login, Focus::Password) => Focus::Username,
                    _ => Focus::Username,
                };
            ...
            KeyCode::Enter => {
                if let Some(a) = self.submit()? {
                    return Ok(Some(crate::tui::EventResponse::Continue(a)));
                }
                Ok(None)
            }
            KeyCode::Esc => {
            ...
            _ => {
                ...
                match self.focus {
                    Focus::Username => self.username_input.handle_event(&ev),
                    Focus::Password => self.password_input.handle_event(&ev),
                    Focus::Confirm => self.confirm_input.handle_event(&ev),
                };
