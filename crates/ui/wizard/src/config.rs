...
impl KeyBindings {
    ...
    pub fn get_scoped(
        ...
        {
            let mut expanded: HashMap<Vec<KeyEvent>, Action> = HashMap::new();
            for (seq, action) in merged.into_iter() {
                for alias_seq in expand_key_sequence_aliases(&seq) {
                    expanded.insert(alias_seq, action.clone());
                }
            }
            expanded
        }
...
pub fn normalize_key_event(key_event: KeyEvent) -> KeyEvent {
    let mut ev = key_event;
    match (ev.code, ev.modifiers) {
        // Ctrl-H is often sent/treated as Backspace by terminals
        (KeyCode::Char('h') | KeyCode::Char('H'), KeyModifiers::CONTROL) => {
            ev.code = KeyCode::Backspace;
            ev.modifiers = KeyModifiers::empty();
        }
        // Ctrl-M is often sent/treated as Enter (CR)
        (KeyCode::Char('m') | KeyCode::Char('M'), KeyModifiers::CONTROL) => {
            ev.code = KeyCode::Enter;
            ev.modifiers = KeyModifiers::empty();
        }
        // Ctrl-I is often sent/treated as Tab
        (KeyCode::Char('i') | KeyCode::Char('I'), KeyModifiers::CONTROL) => {
            ev.code = KeyCode::Tab;
            ev.modifiers = KeyModifiers::empty();
        }
        // Ctrl-[ is often sent/treated as Esc
...
/// Return equivalent key events that should be treated as aliases for matching.
/// The returned vector always includes the original event first.
pub fn aliases_for_event(ev: &KeyEvent) -> Vec<KeyEvent> {
    let mut v = vec![ev.clone()];
    match (ev.code, ev.modifiers) {
        (KeyCode::Backspace, m) if m.is_empty() => {
            v.push(KeyEvent::new(KeyCode::Char('h'), KeyModifiers::CONTROL));
        }
        (KeyCode::Enter, m) if m.is_empty() => {
            v.push(KeyEvent::new(KeyCode::Char('m'), KeyModifiers::CONTROL));
        }
        (KeyCode::Tab, m) if m.is_empty() => {
            v.push(KeyEvent::new(KeyCode::Char('i'), KeyModifiers::CONTROL));
        }
        (KeyCode::Esc, m) if m.is_empty() => {
            v.push(KeyEvent::new(KeyCode::Char('['), KeyModifiers::CONTROL));
        }
        (KeyCode::Char('h') | KeyCode::Char('H'), KeyModifiers::CONTROL) => {
            v.push(KeyEvent::new(KeyCode::Backspace, KeyModifiers::empty()));
        }
        (KeyCode::Char('m') | KeyCode::Char('M'), KeyModifiers::CONTROL) => {
            v.push(KeyEvent::new(KeyCode::Enter, KeyModifiers::empty()));
        }
        (KeyCode::Char('i') | KeyCode::Char('I'), KeyModifiers::CONTROL) => {
            v.push(KeyEvent::new(KeyCode::Tab, KeyModifiers::empty()));
        }
        (KeyCode::Char('['), KeyModifiers::CONTROL) => {
            v.push(KeyEvent::new(KeyCode::Esc, KeyModifiers::empty()));
        }
        _ => {}
    }
    v
...
/// Expand a key sequence with alias variants (cartesian product of per-event aliases).
pub fn expand_key_sequence_aliases(seq: &[KeyEvent]) -> Vec<Vec<KeyEvent>> {
    fn helper(parts: &[KeyEvent], acc: &mut Vec<Vec<KeyEvent>>, current: &mut Vec<KeyEvent>) {
        if parts.is_empty() {
            acc.push(current.clone());
            return;
        }
        let first = &parts[0];
        for alt in aliases_for_event(first).into_iter() {
            current.push(alt);
            helper(&parts[1..], acc, current);
            current.pop();
        }
    }
    let mut out = Vec::new();
    let mut cur = Vec::new();
    helper(seq, &mut out, &mut cur);
    out
}
