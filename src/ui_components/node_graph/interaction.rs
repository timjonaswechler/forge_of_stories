...
use super::{
    context::{GraphChange, LinkValidationCallback},
    coords::{get_screen_space_pin_coordinates, screen_space_to_grid_space},
...
pub(crate) fn process_clicks(
    ...
    // --- Linksklick ---
    if ui_state.persistent.interaction_state.left_mouse_clicked {
        if let Some(pin_idx) = ui_state.frame.hovered_pin_index {
            // Klick auf Pin -> Versuche, Link-Erstellung zu starten
            begin_link_creation(ui_state, storage, pin_idx);
        } else if let Some(node_idx) = ui_state.frame.hovered_node_index {
            ...
            // if self.ui_state.frame.active_pin.map_or(true, |p_id| self.pins.get(&p_id).map_or(true, |p| p.ui_state.persistent.parent_node_idx != node_idx)) {
            begin_node_selection(ui_state, node_idx);
            // }
        } else if let Some(link_idx) = ui_state.frame.hovered_link_idx {
            // Klick auf Link -> Starte Link-Interaktion (Select/Detach)
            begin_link_interaction(ui_state, storage, settings, link_idx);
        } else {
            // Klick ins Leere -> Starte Box-Selektion
            begin_canvas_interaction(ui_state, false);
        }
    }
    // --- Alternativklick (z.B. Mitte, Rechts) ---
    else if ui_state.persistent.interaction_state.alt_mouse_clicked {
        // Klick ins Leere mit Alt -> Starte Panning
        if ui_state.frame.hovered_node_index.is_none()
            && ui_state.frame.hovered_pin_index.is_none()
            && ui_state.frame.hovered_link_idx.is_none()
        {
            begin_canvas_interaction(ui_state, true);
        }
        // Optional: Alt-Klick auf Node/Pin/Link für Kontextmenü etc.
    }
}

fn begin_link_interaction(
    ui_state: &mut GraphUiStateManager,
    storage: &mut GraphStorage,
    settings: &NodesSettings,
    link_id: usize,
    ...
    let pin_is_hovered_and_detachable = ui_state
        .frame
        .hovered_pin_index
        .map(|pin_id| {
            // Prüft ob der Pin zum Link gehört und detachbar ist
            (ui_state.frame.hovered_pin_flags
                & AttributeFlags::EnableLinkDetachWithDragClick as usize
                != 0)
                && storage.links.get(&link_id).map_or(false, |l| {
    ...
    let detach_with_modifier = ui_state
        .persistent
        .interaction_state
    ...
    if pin_is_hovered_and_detachable {
        if let Some(pin_id) = ui_state.frame.hovered_pin_index {
            begin_link_detach(ui_state, storage, link_id, pin_id);
            return;
        }
    }
    ...
    if detach_with_modifier && !pin_is_hovered_and_detachable {
        ...
        if let Some(link) = storage.links.get(&link_id) {
            ...
            if let (Some(start_pin), Some(end_pin)) = (
                ...
                let dist_start_sq =
                    pos_start.distance_sq(ui_state.persistent.interaction_state.mouse_pos);
                let dist_end_sq =
                    pos_end.distance_sq(ui_state.persistent.interaction_state.mouse_pos);
                ...
                begin_link_detach(ui_state, storage, link_id, closest_pin_idx);
                return;
            }
        }
    }
    ...
    begin_link_selection(ui_state, link_id);
}

fn begin_link_creation(
    ui_state: &mut GraphUiStateManager,
    storage: &mut GraphStorage,
    pin_id: usize,
) {
    // Nur starten, wenn noch keine Interaktion läuft und der Pin existiert
    if ui_state.persistent.click_interaction_type == ClickInteractionType::None {
        if let Some(pin) = storage.pins.get(&pin_id) {
                ...
                || (pin.spec.flags & AttributeFlags::EnableLinkCreationOnSnap as usize != 0)
            {
                ...
                ui_state.persistent.click_interaction_state.link_creation =
                ...
                ui_state.persistent.click_interaction_type = ClickInteractionType::LinkCreation;
                ui_state.frame.element_state_change.link_started = true;
            } else {
                bevy::log::trace!(
...
fn begin_link_selection(ui_state: &mut GraphUiStateManager, link_id: usize) {
    if ui_state.persistent.click_interaction_type == ClickInteractionType::None
        || (ui_state.persistent.click_interaction_type == ClickInteractionType::Link
            && !ui_state.persistent.selected_link_indices.contains(&link_id))
    {
        ui_state.persistent.click_interaction_type = ClickInteractionType::Link;
        ui_state.persistent.selected_node_indices.clear(); // Nur Links selektieren
        ui_state.persistent.selected_link_indices.clear();
        ui_state.persistent.selected_link_indices.push(link_id);
    } else if ui_state.persistent.click_interaction_type == ClickInteractionType::Link {
        // Optional: Bei Klick auf bereits selektierten Link -> Deselection?
        // ui_state.persistent.selected_link_indices.clear();
        // ui_state.persistent.click_interaction_type = ClickInteractionType::None;
    }
}

fn begin_node_selection(ui_state: &mut GraphUiStateManager, node_id: usize) {
    if ui_state.persistent.click_interaction_type != ClickInteractionType::None {
        return;
    } // Nur wenn keine andere Interaktion läuft

    ui_state.persistent.click_interaction_type = ClickInteractionType::Node;

    // Check for multi-selection (Shift-Click, Ctrl-Click, etc.) - Nicht implementiert hier
    // Standardverhalten: Clear und Select
    if !ui_state.persistent.selected_node_indices.contains(&node_id) {
        ui_state.persistent.selected_node_indices.clear();
        ui_state.persistent.selected_link_indices.clear(); // Auch Links deselektieren
        ui_state.persistent.selected_node_indices.push(node_id);
        ui_state.frame.just_selected_node = true; // Markiere für Detail View Update etc.
    } else {
        // Node war bereits selektiert. Optional: Bei erneutem Klick nicht wieder auswählen oder Modifikatoren prüfen.
        ui_state.frame.just_selected_node = false; // Nicht *gerade eben* selektiert
    }
        ...
        .position(|x| *x == node_id)
    {
        ...
        let id_to_move = ui_state.persistent.node_depth_order.remove(pos);
        ui_state.persistent.node_depth_order.push(id_to_move);
    } else {
        ...
        ui_state.persistent.node_depth_order.push(node_id); // Vorsichtshalber hinzufügen
    }
}

// Startet Link-Erstellung vom *anderen* Pin, nachdem der Original-Link entfernt wurde
fn begin_link_detach(
    ui_state: &mut GraphUiStateManager,
    storage: &mut GraphStorage,
    link_id: usize,
    detach_pin_idx: usize,
) {
    if ui_state.persistent.click_interaction_type == ClickInteractionType::None {
        if let Some(link_to_detach) = storage.links.get(&link_id) {
            // Nur lesen
            ...
            if storage.pins.contains_key(&other_pin_id) {
...
fn begin_canvas_interaction(ui_state: &mut GraphUiStateManager, start_panning: bool) {
    if ui_state.persistent.click_interaction_type == ClickInteractionType::None {
        if start_panning {
            ui_state.persistent.click_interaction_type = ClickInteractionType::Panning;
        } else {
            // Starte Box Selection
            ui_state.persistent.click_interaction_type = ClickInteractionType::BoxSelection;
            ui_state.persistent.selected_node_indices.clear(); // Auswahl löschen beim Starten
            ui_state.persistent.selected_link_indices.clear();
            ui_state.persistent.click_interaction_state.box_selection = egui::Rect::from_min_max(
                ui_state.persistent.interaction_state.mouse_pos, // Start an der Mausposition
                ui_state.persistent.interaction_state.mouse_pos,
            );
        }
    }
}

pub(crate) fn translate_selected_nodes(
    ui_state: &mut GraphUiStateManager,
    storage: &mut GraphStorage,
) {
    // Nur ausführen, wenn gezogen wird UND eine Node-Interaktion läuft
    if ui_state.persistent.interaction_state.left_mouse_dragging
        && ui_state.persistent.click_interaction_type == ClickInteractionType::Node
    {
        // Delta in Screen Space
        let delta_screen = ui_state.persistent.interaction_state.mouse_delta;
        ...
        if delta_grid.length_sq() > 0.0 {
            ...
            for node_id in &ui_state.persistent.selected_node_indices {
...
pub(crate) fn box_selector_update_selection(
    ui_state: &mut GraphUiStateManager,
    storage: &mut GraphStorage,
    settings: &NodesSettings,
) {
    ...
    ui_state
        ...
        .box_selection
        .max = ui_state.persistent.interaction_state.mouse_pos;
    let box_rect_screen = ui_state.persistent.click_interaction_state.box_selection;
    ...
    let box_rect_grid = egui::Rect::from_min_max(
        screen_space_to_grid_space(ui_state, normalized_box_screen.min),
        screen_space_to_grid_space(ui_state, normalized_box_screen.max),
    );

    let previous_selected_nodes = ui_state.persistent.selected_node_indices.clone(); // Merke vorherige Auswahl für Reihenfolge
    ui_state.persistent.selected_node_indices.clear(); // Leere aktuelle Auswahl
    ui_state.persistent.selected_link_indices.clear(); // Auch Links leeren

    // --- Node Selection ---
    for (id, node) in storage.nodes.iter() {
        // Prüfe Überschneidung im Grid Space
        if box_rect_grid.intersects(node.state.rect) {
            ui_state.persistent.selected_node_indices.push(*id);
        }
    }
    // Optional: Behalte ursprüngliche Selektionsreihenfolge bei, falls Nodes wieder selektiert werden
    ui_state.persistent.selected_node_indices.sort_by_key(|id| {
    ...
    for (id, link) in storage.links.iter() {
        ...
        if let (Some(start_pin), Some(end_pin)) = (
            ...
            if storage.nodes.contains_key(&start_pin.state.parent_node_idx)
                && storage.nodes.contains_key(&end_pin.state.parent_node_idx)
            {
                ...
                if rectangle_overlaps_link(
                    ...
                    ui_state.persistent.selected_link_indices.push(*id);
                }
            }
        }
    }
...
pub(crate) fn click_interaction_update(
    ...
    match ui_state.persistent.click_interaction_type {
        ClickInteractionType::LinkCreation => {
            // Hole ID des ggf. modifizierten Links
            let modifying_link_id = ui_state
                .persistent
                .click_interaction_state
                .link_creation
                .modifying_link_id;
            let start_pin_id = ui_state
                .persistent
                .click_interaction_state
                .link_creation
                .start_pin_idx
            ...
            if let Some(hovered_pin_id) = ui_state.frame.hovered_pin_index {
                let duplicate_link_id = find_duplicate_link(&storage, start_pin_id, hovered_pin_id);
            ...
            if ui_state.persistent.interaction_state.left_mouse_released {
                if let Some(end_pin_id) = snapped_pin_id {
                    ...
                    if let Some(original_link_id) = modifying_link_id {
                        ...
                        if let Some(removed_link_data) = storage.links.remove(&original_link_id) {
                        ...
                        let new_link_state = LinkState {
                            style: settings.style.format_link(new_link_spec.style.clone()),
                        };
                        // Füge den neuen Link zum internen Zustand hinzu
                        if let Some(_existing_link) = storage.links.insert(
                            new_link_id,
                            Link {
                                spec: new_link_spec,
                ...
                ui_state.persistent.click_interaction_state.link_creation = Default::default();
        ...
        ClickInteractionType::BoxSelection => {
            box_selector_update_selection(ui_state, storage, settings); // Aktualisiert die Auswahl basierend auf der Box
            if ui_state.persistent.interaction_state.left_mouse_released {
                // Box-Selektion beendet
                ui_state.persistent.click_interaction_type = ClickInteractionType::None;
                // Bringt neu ausgewählte Nodes in der Tiefenordnung nach oben (optional)
                let s = ui_state.persistent.selected_node_indices.clone();
                ui_state
                    .persistent
                    .node_depth_order
                    .retain(|id| !s.contains(id));
                ui_state.persistent.node_depth_order.extend(s);
            }
        }
        ClickInteractionType::Node => {
            translate_selected_nodes(ui_state, storage); // Bewegt ausgewählte Nodes
            if ui_state.persistent.interaction_state.left_mouse_released {
                // Node-Drag beendet
                ui_state.persistent.click_interaction_type = ClickInteractionType::None;
            }
        ...
        ClickInteractionType::Panning => {
            ...
            if ui_state.persistent.interaction_state.alt_mouse_dragging {
                ui_state.persistent.panning += ui_state.persistent.interaction_state.mouse_delta;
            // Update Panning
            }
            ...
            else if !ui_state.persistent.interaction_state.alt_mouse_dragging
                && !ui_state.persistent.interaction_state.alt_mouse_clicked
            {
                ui_state.persistent.click_interaction_type = ClickInteractionType::None;
            }
        }
...
fn should_link_snap_to_pin(
    ...
    let Some(start_pin) = storage.pins.get(&start_pin_id) else {
        return false;
    };
    let Some(end_pin) = storage.pins.get(&hovered_pin_id) else {
        return false;
    };
    ...
    if !link_validator(&start_pin.spec, &end_pin.spec, storage, settings) {
        return false;
    }
...
pub(crate) fn handle_delete(ui_state: &mut GraphUiStateManager, storage: &mut GraphStorage) {
    let mut links_to_remove_ids: Vec<usize> = ui_state
        .persistent
        .selected_link_indices
        .drain(..)
        .collect();
    let nodes_to_remove_ids: Vec<usize> = ui_state
        .persistent
    ...
    if !nodes_to_remove_ids.is_empty() {
        ...
        for (link_id, link) in storage.links.iter() {
    ...
    for link_id in &links_to_remove_ids {
        ...
        if let Some(removed_link) = storage.links.remove(link_id) {
            // Sende das NEUE Event mit den Pin IDs
            ui_state.frame.graph_changes.push(GraphChange::LinkRemoved {
    ...
    for node_id in nodes_to_remove_ids {
        ...
        if storage.nodes.remove(&node_id).is_some() {
            ui_state
                .frame
                .graph_changes
