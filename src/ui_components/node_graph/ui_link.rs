...
use bevy_egui::egui;
// use bevy_inspector_egui::bevy_egui; // Entfernen, falls nicht genutzt
...
#[derive(Debug, Clone)] // <-- Clone hinzugefügt (brauchte LinkState)
...
#[derivative(Debug)]
pub struct Link {
    pub spec: LinkSpec,   // Ist jetzt Clone
    pub state: LinkState, // Ist jetzt Clone
}
...
// --- Rest der Datei (BezierCurve etc.) bleibt unverändert ---
#[derive(Debug)]
...
fn eval_inplicit_line_eq(p1: &egui::Pos2, p2: &egui::Pos2, p: &egui::Pos2) -> f32 {
    // ... (Implementierung unverändert) ...
    (p2.y - p1.y) * p.x + (p1.x - p2.x) * p.y + (p2.x * p1.y - p1.x * p2.y) // Korrigierte Formel
}
